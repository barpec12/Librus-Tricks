...
class SynergiaUser:
    ...
    async def revalidate_root(self):
        httpx_auth_session = httpx.AsyncClient()
        new_tokens = await httpx_auth_session.post(
            OAUTHURL,
            data={
    ...
    async def revalidate_user(self):
        httpx_auth_session = httpx.AsyncClient()
        new_token = await httpx_auth_session.get(
...
async def authorizer(email, password):
    """
    ...
    """
    httpx_auth_session = httpx.AsyncClient()
    site = await httpx_auth_session.get(LIBRUSLOGINURL)
    soup = BeautifulSoup(site.text, 'html.parser')
    csrf = soup.find('meta', attrs={'name': 'csrf-token'})['content']
    login_response_redirection = await httpx_auth_session.post(
    ...
    if login_response_redirection.status_code != 200:
        if login_response_redirection.status_code == 403:
            raise Exception(login_response_redirection.text) # TODO: require append right exception
        else:
            raise Exception(login_response_redirection.text) # TODO: require append right exception

    redirection_addr = login_response_redirection.json()['redirect']
    redirection_response = await httpx_auth_session.get(redirection_addr, allow_redirects=False)
    oauth_code = redirection_response.headers['location'].replace('http://localhost/bar?code=', '')

    synergia_root_response = await httpx_auth_session.post(
        OAUTHURL,
        data={
    ...
    synergia_users_response = await httpx_auth_session.get(SYNERGIAAUTHURL,
